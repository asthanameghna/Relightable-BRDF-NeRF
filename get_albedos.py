...
##################### This file renders 360-deg albedos.png files for any trained model - both multipose+single_lightdir AND singlepose+multi_lightdir ###################
...
def render_rays(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d):
        ...
        rgb_map = tf.reduce_sum(
            weights[..., None] * bottleneck, axis=-2)  # [N_rays, 3]
...
testimgdir = os.path.join(basedir, expname, 'poses_albedos')
...
for i in range(0,20):
    ...
    saveimg_dir_r = os.path.join(
            basedir, expname, 'poses_albedos/render_p{:02d}.png'.format(i + 1))
    imageio.imwrite(saveimg_dir_r, architecture.to8b(albedo))
