...
##################### This file renders 360-deg normals.png files for any trained model - both multipose+single_lightdir AND singlepose+multi_lightdir ###################
...
def render_rays(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d):
        ...
        rgb_map = tf.reduce_sum(
            weights[..., None] * n_norm, axis=-2)  # [N_rays, 3]
...
testimgdir = os.path.join(basedir, expname, 'poses_normals')
...
for i in range(0,20):
    ...
    normal = rgb / np.linalg.norm((rgb), axis=-1, keepdims=True)
    normal_new = 0.5 * (normal + 1)
    saveimg_dir_r = os.path.join(
            basedir, expname, 'poses_normals/render_p{:02d}.png'.format(i + 1))
    imageio.imwrite(saveimg_dir_r, architecture.to8b(normal_new))
